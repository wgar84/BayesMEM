...
runciePost $ diagW <-
  llply (runciePost $ ext, DiagW,
         vcv.terminal.list = llply (OneDef[41:46],
           function (L) L $ ml.vcv),
         sample.size = Aux $ sample.size [41:46], 
...
runciePost $ quantile <-
  foreach(i = 1:4) %dopar%
DiagQuantilePop(161, runciePost $ ext [[i]], OneDef, tree = Tree [[5]])
